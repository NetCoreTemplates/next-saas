  ...
  return <AppShell workspaceName={data?.workspace?.name}>
    ...
    <FeatureGate feature="files.basic" entitlements={data?.entitlements}>
      <div className="mb-5 grid gap-4 md:grid-cols-2">
        {[{icon:FileCheck2,label:'Documents stored',usage:documentUsage,format:(n?:number)=>(n??0).toLocaleString()},{icon:HardDrive,label:'Storage used',usage:storageUsage,format:bytes}].map(({icon:Icon,label,usage,format})=><Panel key={label} className="p-5"><div className="flex items-center justify-between"><span className="grid h-10 w-10 place-items-center rounded-xl bg-blue-50 text-[#0b5cff] dark:bg-blue-400/10 dark:text-blue-300"><Icon className="h-5 w-5"/></span><span className="text-xs font-medium text-slate-500">{label}</span></div><div className="mt-5 flex items-end justify-between"><div><p className="tabular text-2xl font-semibold tracking-[-.04em]">{format(usage?.usedUnits)}</p><p className="mt-1 text-xs text-slate-400">of {usage?.allowance==null?'unlimited':format(usage.allowance)}</p></div><span className={`tabular text-sm font-semibold ${quotaText[quotaTone(usage?.percentUsed)]}`}>{usage?.allowance==null?'—':`${usage?.percentUsed ?? 0}%`}</span></div><Meter percent={usage?.allowance==null?0:usage?.percentUsed} label={`${label} used`} className="mt-4 h-2"/></Panel>)}
      </div>
      ...
      <Panel className={`overflow-hidden transition ${dragging?'border-[#0b5cff] ring-2 ring-[#0b5cff]/20':''}`}><div className="flex flex-col gap-4 border-b border-slate-200 px-6 py-5 sm:flex-row sm:items-center sm:justify-between dark:border-white/10"><div><h2 className="font-semibold">Organization files</h2><p className="mt-1 text-xs text-slate-400">Visible to every member of this organization. Drag files anywhere onto this panel to upload.</p></div><label className="relative"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400"/><input value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void load(search)}} placeholder="Search files" className="w-full rounded-[10px] border-slate-200 bg-white py-2 pl-9 pr-3 text-sm dark:border-white/10 dark:bg-white/5 sm:w-64"/></label></div>
        {loading?<div className="grid min-h-52 place-items-center"><span className="h-7 w-7 animate-spin rounded-full border-2 border-blue-100 border-t-[#0b5cff]"/></div>:files.length?<div className="divide-y divide-slate-100 dark:divide-white/5">{files.map(file=><div key={file.id} className="flex items-center gap-4 px-6 py-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-500 dark:bg-white/5"><FileText className="h-5 w-5"/></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{file.name}</p><p className="mt-1 text-xs text-slate-400">{bytes(file.byteLength)} · {file.createdDate?new Date(file.createdDate).toLocaleString():'—'} · <span className="font-mono">{file.sha256?.slice(0,12)}…</span></p></div><StatusPill tone={file.status==='Available'?'green':'slate'}>{file.status}</StatusPill><button onClick={()=>void download(file)} className="grid h-9 w-9 place-items-center rounded-lg text-slate-400 hover:bg-blue-50 hover:text-[#0b5cff] dark:hover:bg-blue-400/10" title="Download" aria-label={`Download ${file.name}`}><Download className="h-4 w-4"/></button><button onClick={()=>void remove(file)} className="grid h-9 w-9 place-items-center rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-400/10" title="Delete" aria-label={`Delete ${file.name}`}><Trash2 className="h-4 w-4"/></button></div>)}</div>:<div className="grid min-h-64 place-items-center p-8 text-center"><div><CloudUpload className="mx-auto h-8 w-8 text-slate-300"/><h3 className="mt-4 font-semibold">{search?'No files match your search':'Upload your first document'}</h3><p className="mt-2 text-sm text-slate-500">{search?'Try a different name, or clear the search to see every file.':'Drag files here, or choose several at once. Each file counts toward your document and storage allowances.'}</p>{!search&&!readOnly&&<button type="button" onClick={()=>fileInput.current?.click()} disabled={busy} className="mt-5 inline-flex items-center gap-2 rounded-[10px] border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800 transition hover:border-slate-400 disabled:opacity-60 dark:border-white/15 dark:bg-white/5 dark:text-white"><CloudUpload className="h-4 w-4"/>Choose files</button>}</div></div>}
      </Panel>
      </div>
    </FeatureGate>
  </AppShell>
