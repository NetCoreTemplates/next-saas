...
export default function AppShell({ children, workspaceName = 'My organization' }: { children: React.ReactNode, workspaceName?: string }) {
  ...
  const links = activeWorkspace?.workspace?.kind === 'Individual' ? allLinks.filter(x => x.href !== '/team') : allLinks
...
export function StatusPill({ children, tone='green' }: {children:React.ReactNode,tone?:'green'|'blue'|'amber'|'red'|'slate'}) { const colors={green:'bg-emerald-50 text-emerald-700 dark:bg-emerald-400/10 dark:text-[#86efcd]',blue:'bg-blue-50 text-[#0b5cff] dark:bg-blue-400/10 dark:text-blue-300',amber:'bg-amber-50 text-amber-700 dark:bg-amber-400/10 dark:text-amber-300',red:'bg-red-50 text-red-700 dark:bg-red-400/10 dark:text-red-300',slate:'bg-slate-100 text-slate-600 dark:bg-white/5 dark:text-slate-300'}; return <span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.08em] ${colors[tone]}`}>{children}</span> }
